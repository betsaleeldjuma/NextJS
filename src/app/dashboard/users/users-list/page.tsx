              ...
              {users.map((user) => (
                <tr key={user.id} className='border-t hover:bg-gray-50'>
                  ...
                  <td className='py-4 px-4'>
                    <span className={`user.status === 'Active' ? 'text-green-500' : user.status === 'Inactive' ? 'text-yellow-500' : 'text-red-500`}>{user.status}</span>
                  </td>
                  <td className='py-4 px-4 text-center'>
                    <button className='px-4 py-2 text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors'>Edit</button>
                    <button className='ml-2 px-4 py-2 text-white bg-blue-600 rounded-lg hover:blue-700 transition-colors'>View</button>
                  </td>
                </tr>
              ))}
            </tbody>

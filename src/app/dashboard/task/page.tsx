...
const Task = () => {
  const task = [
    {
      ...
      description: 'Revise the homepgae text to reflect recent changes.',
      status: 'Pending',
      dueDate: '2025-02-20'
    }, {
      id: 3,
  ...
  return (
    <div className='min-h-screen bg-grandient-to-br from-gray-50 to-blue-50 p-8'>
          ...
          {task.map(task => (
            <div key={task.id} className='flex items-center justify-between bg-white rounded-lg shadow-md p-6 border-l-4' style={{borderColor: task.status === 'Completed' ? 'green' : task.status === 'In Progress' ? 'yellow':'red'}}>
              <div className="flex flex-col">
                ...
                <p className='text-sm text-gray-600 mt-2'>Due Data: {task.dueDate}</p>
              </div>
